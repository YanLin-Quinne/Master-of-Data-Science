...
library(mlbench)
data("BreastCancer")
dim(BreastCancer) #699rows 11colunms 
...
summary(logreg1_fit)
test_error_red = general_cv(MyCancer_red[,1:pstar], MyCancer_red[,pstar+1], 
...
summary(glm(Class~Cl.thickness+Cell.shape+Marg.adhesion+Bare.nuclei+Bl.cromatin+Normal.nu cleoli+Mitoses,
            family='binomial',data=MyCancer)) 
lr_red_train=glm(Class~Cl.thickness+Cell.shape+Marg.adhesion+Bare.nuclei+Bl.cromatin+Normal .nucleoli+Mitoses,
                 data=MyCancer[train_set,],family='binomial')
head(lr_red_train)
...
## Use 10-fold to verify test error in the LDA model (7-variable) l
da_fold_error = function(X, y, test_data) {
  Xy = data.frame(X, y=y)
...
lda_train=lda(Class~Cl.thickness+Cell.shape+Marg.adhesion+Bare.nuclei+Bl.cromatin+Normal.nu cleoli+Mitoses,
              data=MyCancer_red[train_set,]) 
...
test_error_qda = general_cv(MyCancer_red[,1:pstar], MyCancer_red[,pstar+1], 
                            fold_index, qda_fold_error)

#validation set method test error for QDA model 
qda_train=qda(Class~Cl.thickness+Cell.shape+Marg.adhesion+Bare.nuclei+Bl.cromatin+Normal.n ucleoli+Mitoses, 
              data=MyCancer_red[train_set,]) 
